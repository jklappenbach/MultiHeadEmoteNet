...
# Load the dataset
  # or paste the function above in your script

h5_path = "data/h5_datasets/emotion-detection-fer.h5"
...
# Peek one batch from each loader to get shapes
train_image, train_label = next(iter(train_loader))
val_image, val_label = next(iter(val_loader))
test_image, test_label = next(iter(test_loader))
print(f"Train batch: Image shape {tuple(train_image.shape)}, Label shape {tuple(train_label.shape)}")
print(f"Validation batch: Image shape {tuple(val_image.shape)}, Label shape {tuple(val_label.shape)}")
print(f"Test batch: Image shape {tuple(test_image.shape)}, Label shape {tuple(test_label.shape)}")
